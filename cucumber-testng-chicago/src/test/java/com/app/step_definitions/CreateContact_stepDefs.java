...
public class CreateContact_stepDefs {
	...
	public void i_validate_new_created_contact_info(String prefix, String firstName, String lastName,
		...
		assertTrue(cPage.contactTitle.getText().contains(lastName.toUpperCase()),
				"Contact title validation: " + cPage.contactTitle.getText());
	...
	public void i_validate_contacts_with_data_in_excel_sheet_and(String path, String sheet) {
		...
		for (int i = 1; i < ExcelUtils.getUsedRowsCount(); i++) {
			...
			String tempContact = ExcelUtils.getCellData(i, 0);
			...
			dash.searchInput.sendKeys(tempContact + Keys.ENTER);
					...
					List <WebElement> result = Driver.getDriver().findElements(By.xpath(xpath));
					if (result.size() > 0) {
